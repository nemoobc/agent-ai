...
4. **AUTO-DELEGATION** — DEV calls sub-agents (architect, coder, tester, auditor, fixer, memory) automatically.
...
9. **FULL VERIFICATION** — report DONE only after every gate is green (`/verify`: lint, self-test, e2e, demo, update, mutation, bench, audit, doctor).
...
- **Self-proving** — `tests/mutation.sh` deliberately breaks the kit 6 ways and demands a gate catches each one; `tests/bench.sh` detects gate slowdown (drift); `make verify` runs every gate in one command.
- **Security** — audit-full secret scan, red-team attack simulation, git-guard blocks secrets/merge markers/debug before commit.
...
bash tests/mutation.sh     # proof: 6 deliberate breakages caught by gates
