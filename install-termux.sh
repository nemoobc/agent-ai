#!/usr/bin/env bash
# ============================================================
#  install-termux.sh — agent-ai untuk TERMUX Android SAJA.
#  Tanpa root. Backup otomatis. Idempoten (aman diulang).
#
#  Cara pakai:
#    bash install-termux.sh              # dari dalam folder repo
#    bash install-termux.sh --check      # cek status tanpa ubah apa-apa
# ============================================================
set -u
...
if [ "${AGENT_OS:-}" = "linux" ]; then
  die "ini skrip TERMUX. Kamu di Linux? pakai install-linux.sh"
fi
if [ -z "${PREFIX:-}" ] || ! printf '%s' "$PREFIX" | grep -q "com.termux"; then
  if [ ! -d "/data/data/com.termux" ]; then
    die "bukan Termux ini. Linux? pakai install-linux.sh"
  fi
fi
say "OS: termux (valid)"
# ---------- 2. cek tool ----------
...
if command -v opencode >/dev/null 2>&1; then
  ...
  say "  Termux: pasang binary dulu: https://github.com/nemoobc/opencode-termux"
fi

say ""
say "SELESAI. Jalankan: opencode"
say "Cek lain waktu: bash install-termux.sh --check"
