#!/usr/bin/env bash
# ============================================================
#  install.sh — Agent: pasang config OpenCode (Linux + Termux)
#  Tanpa root. Backup otomatis. Idempoten (aman diulang).
#
#  Cara pakai:
#    bash install.sh              # dari dalam folder repo
#    bash install.sh --check      # cek status tanpa ubah apa-apa
# ============================================================
set -u
...
# ---------- 1. deteksi OS ----------
OS="linux"
if [ -n "${PREFIX:-}" ] && [ -d "$PREFIX" ] && printf '%s' "$PREFIX" | grep -q "com.termux"; then
  OS="termux"
elif [ "${OSTYPE:-}" = "android" ] || [ -d "/data/data/com.termux" ]; then
  OS="termux"
fi
say "OS terdeteksi: $OS"
...
if command -v opencode >/dev/null 2>&1; then
  ...
  if [ "$OS" = "termux" ]; then
    say "  Termux: ikuti https://github.com/nemoobc/opencode-termux"
  else
    say "  Linux: npm install -g opencode-ai  ATAU  curl -fsSL https://opencode.ai/install | bash"
  fi
fi

say ""
say "SELESAI. Jalankan: opencode"
say "Cek lain waktu: bash install.sh --check"
